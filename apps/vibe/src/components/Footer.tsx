...
export default function Footer({ locale }: { locale: Locale }) {
  ...
  return (
    <footer className="border-border border-t px-4 pt-8 pb-[calc(5rem+var(--safe-area-bottom))] text-center text-foreground-muted text-xs sm:pb-8">
      <nav className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
        ...
        <Link className="hover:text-foreground" href={`/${locale}/refund`}>
          {nav.refund}
        </Link>
      </nav>
      ...
      <p className="mt-6 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-foreground-muted">
      ...
      <p className="mt-2 text-foreground-muted">© 2026 {SITE_NAME[locale]} · vibe.sobok.cc</p>
    </footer>
  )
}
