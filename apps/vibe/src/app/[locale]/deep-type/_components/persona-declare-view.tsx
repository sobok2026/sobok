...
export function PersonaDeclareView({ onDeclare, onGuide, ui }: PersonaDeclareViewProps) {
  return (
    <main className="flex flex-1 flex-col justify-center bg-background px-safe py-10 text-foreground" id="main-content">
      <div className="mx-auto w-full max-w-2xl py-4">
        <h1 className="font-black text-3xl leading-tight">{ui.declareTitle}</h1>
        <p className="mt-4 text-foreground-secondary leading-8">{ui.declareBody}</p>

        <div className="mt-8 grid grid-cols-2 gap-2 sm:grid-cols-4">
          {PERSONA_CODES.map((code) => (
            <button
              className={cn(
                'min-h-13 rounded-2xl border border-border bg-surface font-black text-base tabular-nums transition-colors hover:border-brand hover:bg-brand/8 hover:text-accent',
                FOCUS_CLASS_NAME,
              )}
              key={code}
        ...
        <button
          className={cn(
            'mt-3 min-h-14 w-full rounded-2xl border border-border bg-surface px-5 text-left font-bold text-foreground transition-colors hover:border-brand hover:text-accent',
            FOCUS_CLASS_NAME,
          )}
          onClick={onGuide}
          type="button"
        >
          ...
          <span className="mt-0.5 block font-medium text-foreground-muted text-xs">{ui.declareUnknownHint}</span>
        </button>
      </div>
    </main>
  )
