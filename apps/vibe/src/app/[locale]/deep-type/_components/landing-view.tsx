...
export function LandingView({ content, locale, onStart }: LandingViewProps) {
  ...
  return (
    <main className="flex flex-1 flex-col bg-background px-safe pt-10 pb-32 text-foreground" id="main-content">
      <div className="mx-auto w-full max-w-3xl">
        <p className="font-black text-foreground-secondary text-sm tracking-tight">{DEEP_TYPE_BRAND_NAME[locale]}</p>

        <header className="mt-8">
          <span className="font-bold text-accent text-sm">{landing.kicker}</span>
          <h1 className="mt-3 text-balance font-black text-4xl leading-tight sm:text-5xl">{ui.landingTitle}</h1>
          <p className="mt-5 break-prose text-foreground-secondary leading-8">{ui.landingSubtitle}</p>
        </header>

        <figure className="mt-8 overflow-hidden rounded-4xl border border-border bg-surface-2">
          <Image
            alt=""
        ...
        <dl className="mt-6 grid grid-cols-3 gap-2">
          {landing.facts.map((fact) => (
            <div className="rounded-3xl border border-border bg-surface px-3 py-4 text-center" key={fact.label}>
              <dt className="text-foreground-muted text-xs">{fact.label}</dt>
        ...
        <Section title={landing.getsTitle}>
          <ol className="mt-5 grid gap-3">
            {FREE_DELIVERABLES_KO.map((deliverable, index) => (
              <li className="flex gap-4 rounded-3xl border border-border bg-surface p-5" key={deliverable}>
                <span className="shrink-0 font-black text-accent text-sm tabular-nums">
                  {String(index + 1).padStart(2, '0')}
                </span>
                <div>
                  <p className="font-black">{deliverable}</p>
                  <p className="mt-1 break-prose text-foreground-muted text-sm leading-6">
        ...
        <section className="mt-14 grid gap-3">
          {landing.asks.map((ask) => (
            <div className="rounded-3xl border border-border bg-surface-2/60 p-5" key={ask.question}>
              <h2 className="font-black text-lg leading-8">{ask.question}</h2>
              <p className="mt-2 break-prose text-foreground-secondary text-sm leading-7">{ask.body}</p>
            </div>
          ))}
        </section>
        ...
        <Section title={landing.stepsTitle}>
          <ol className="mt-5 grid gap-2">
            {landing.steps.map((step, index) => (
              <li
                className="flex items-center justify-between gap-4 rounded-3xl border border-border bg-surface px-5 py-4"
                key={step.title}
              >
                ...
                <span className="shrink-0 text-foreground-muted text-xs">{step.duration}</span>
              </li>
            ))}
          </ol>
        </Section>
        ...
        <Link
          className={cn(
            'mx-auto mt-4 flex w-fit items-center rounded-full p-3 text-center font-bold text-foreground-muted text-xs underline underline-offset-4 hover:text-foreground',
            FOCUS_CLASS_NAME,
          )}
        ...
        <div className="mt-10 border-border border-t pt-6">
          <p className="break-prose text-foreground-muted text-xs leading-6">{landing.offerNote}</p>
        </div>
      </div>
...
function CtaBlock({
  ...
  return (
    <div className={className}>
      <Link
        className={cn(
          'flex w-full touch-manipulation items-center justify-center gap-2 rounded-2xl bg-accent py-4 font-black text-base text-white shadow-[0_24px_80px_var(--accent-glow)] transition-colors hover:bg-accent/92 sm:mx-auto sm:max-w-sm',
          FOCUS_CLASS_NAME,
        )}
      ...
      <p className="mt-3 text-balance text-center text-foreground-muted text-sm leading-6">{meta}</p>
    </div>
  )
}
  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-xs">
        <Link
          className={cn(
            'flex w-full touch-manipulation items-center justify-center gap-2 rounded-full bg-accent py-3.5 font-black text-sm text-white transition-colors hover:bg-accent/92',
            FOCUS_CLASS_NAME,
          )}
          href={`/${locale}/deep-type/test`}
          onClick={onStart}
