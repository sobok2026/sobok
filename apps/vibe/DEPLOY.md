...
2. 제품별 최소 권한 schema Environment에서 `stella_stg`와 `deeptype_stg`를 조정한다.
...
- `CLOUDFLARE_API_TOKEN`과 `CLOUDFLARE_ACCOUNT_ID`는 `staging` Environment에 둔다. DB URL은
  `staging-stella-schema`와 `staging-vibe-schema`에 각각 두며 자기 schema job에만 주입한다.
- production schema와 앱 배포는 `main`에서 각각 수동 실행한다. `production-stella-schema`와
  `production-vibe-schema`가 schema URL을 분리하고 앱 배포 workflow에는 DB URL을 전달하지 않는다.
...
`CLOUDFLARE_API_TOKEN`·`CLOUDFLARE_ACCOUNT_ID`·`SOBOK_POSTGRES_URL_DIRECT`는 존재하면 안 된다.
값은 보호된 deployment/schema Environment에만 둔다.
...
- [ ] **CI 크레덴셜 경계** — `CLOUDFLARE_API_TOKEN`·`CLOUDFLARE_ACCOUNT_ID`는
      `production`·`staging` Environment에만, `SOBOK_POSTGRES_URL_DIRECT`는 네 product schema
