...
// Every axis carries four indicators keyed exactly two forward + two reverse. Balanced keying keeps the
// scale symmetric: undifferentiated (straight-line) responding cancels to a true 0 lean (50/50) instead
// of drifting toward a pole, and the axis mean can reach an exact tie. Response options stay ordered
// pole-to-pole, so a forward item runs second-pole → first-pole while a reverse item runs the opposite way.
...
// Every paid visitor receives this same bank. It deepens Inner and Gem with four additional indicators
// per code-driving axis (keyed two forward + two reverse), lifting those axes to eight items each. It is
// deliberately not presented as IRT/CAT or adaptive testing.
