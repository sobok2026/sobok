...
// Core query builder only (tables are imported directly by the query modules), so no relational `schema`
// is passed to drizzle — same shape as @sobok/db's `drizzle({ client })`.
