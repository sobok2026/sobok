...
export default function Footer({ locale }: { locale: Locale }) {
  ...
  return (
    <footer className="border-t border-border px-4 pt-8 pb-[calc(2rem+var(--safe-area-bottom))] text-center text-xs text-foreground-subtle">
      ...
      <p className="mt-4 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-foreground-faint">
        {otherServices('zwds').map((service) => (
