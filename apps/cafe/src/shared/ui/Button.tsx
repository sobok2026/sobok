...
export function TextButton({ danger = false, className, ...props }: ComponentProps<'button'> & { danger?: boolean }) {
  return (
    <button
      type="button"
      {...props}
...
export function InventoryButton(props: ComponentProps<'button'>) {
  return (
    <button
      type="button"
      {...props}
      className="w-full rounded border border-control-line bg-control px-3 py-2.5 text-sm text-brand"
    />
  )
}
