...
function measuringAction(brew: ColdBrew, step: ReturnType<typeof coldBrewStep>, ready: boolean) {
  if (brew.tool && ready) return 'G로 도구를 내려놓고 F로 계량을 확인하세요.'
  if (brew.step === 2) return `Space로 ${COLD_BREW_HOURS}시간 추출을 시작하세요.`
  if (ready) return 'F로 계량을 확인하세요.'
  if (!brew.tool) return `G로 ${COLD_BREW_TOOL_NAMES[step.tool!]}를 집으세요.`
  if (brew.step === 0) return 'Space를 누르고 원두 한 봉을 모두 담으세요.'
