...
export function operationDetails(step: WorkStep): string[] {
  ...
  if ('materialId' in op && op.materialId) details.push(itemName(op.materialId))
  if (op.action === 'charge' && op.gasMaterialId) details.push(itemName(op.gasMaterialId))
  if ('from' in op && 'into' in op) details.push(`${itemName(op.from)} → ${itemName(op.into)}`)
...
export function workProgressLabel(step: WorkStep, progress: number): string {
  if (step.kind === 'condition') return '상태 확인 대기'
  const op = step.operation
