...
import { closingTasks, nextStep } from '../game/progress'
import { expiryAt } from '../game/quality'
import type { Cleaning, ColdBrew, GameState, Preparation, Washing } from '../game/state'
import { WASH_NAMES, WASH_STEPS, washDestination } from '../game/washing'

type Tip = { title: string; action: string; reason: string; fault?: boolean }
...
export function currentTip(state: GameState, panel: StationId | null): Tip {
  ...
  if (state.washing && !cupCount(state.cleaning?.heldCups)) return washingTip(state.washing)
  if (state.cleaning) return cleaningTip(state.cleaning)
  const brew = state.coldBrew
  if (brew && (brew.tool || (!state.preparation && !cup))) return coldBrewTip(state, brew)
  if (state.preparation) return preparationTip(state, state.preparation)
  if (cup?.craft.fault)
    return {
...
function washingTip(wash: Washing): Tip {
...
function cleaningTip(work: Cleaning): Tip {
...
function coldBrewTip(state: GameState, brew: ColdBrew): Tip {
...
function preparationTip(state: GameState, prep: Preparation): Tip {
  if (prep.fault)
    return {
...
function craftTip(state: GameState, cup: NonNullable<GameState['cup']>): Tip {
