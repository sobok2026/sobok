...
export function customizePlan(base: PlannedStep[], custom: Customizations): PlannedStep[] {
  ...
  const plan: PlannedStep[] = base.flatMap((step) => {
    ...
    return [
      {
        ...step,
        operation,
        portion: custom.levels[step.id] === 'less' ? 0.5 : custom.levels[step.id] === 'extra' ? 1.5 : undefined,
        measurement: custom.levels[step.id]
          ? `기본 투입량의 ${custom.levels[step.id] === 'less' ? 50 : 150}%`
          : measurement,
