...
for (const id of Object.keys(sourceQuality)) {
  if (!sourceIds.has(id)) {
    throw new Error(`품질 기준에 원문 재료 '${id}'가 없습니다.`)
  }
}

for (const id of Object.keys(shopInventory.materials)) {
  if (!sourceIds.has(id)) {
    throw new Error(`재고 운영값에 원문 재료 '${id}'가 없습니다.`)
  }
}

for (const id of Object.keys(shopStockRules.materials)) {
  if (!sourceIds.has(id)) {
    throw new Error(`재고 환산값에 원문 재료 '${id}'가 없습니다.`)
  }
}
...
export const INGREDIENTS: Record<string, Ingredient> = Object.fromEntries(
  materials.flatMap((material) => {
    if (!shopStockRules.materials[material.id]) {
      throw new Error(`${material.name}: 재고 환산값이 없습니다.`)
    }
    const supply = shopInventory.materials[material.id]

    if (material.kind === 'utility') {
      if (supply) {
        throw new Error(`${material.name}: 무제한 공급품은 재고 품목으로 등록할 수 없습니다.`)
      }
      if (!shopStockRules.utilityUnits[material.id]) {
        throw new Error(`${material.name}: 공급량 계산 단위가 없습니다.`)
      }
      return []
    }

    if (!supply) {
      throw new Error(`${material.name}: 재고 운영값이 없습니다.`)
    }
