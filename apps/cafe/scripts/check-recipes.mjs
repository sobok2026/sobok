...
try {
  ...
  const result = buildMenu(catalog, await read('menu.json'))
  const menu = Object.values(result.available)
  ...
  console.log(`판매 보류 ${result.unavailable.length}개: 가격·원문 계량·부재료 준비 경로 확인 필요`)
  if (process.argv.includes('--details')) {
    for (const item of result.unavailable) console.log(`- ${item.name}: ${item.reasons.join(' / ')}`)
  }
} catch (error) {
