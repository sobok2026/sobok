...
const CONTEXTS = ['desire', 'evidence', 'protected-value', 'reversible-step'] as const
...
if (values.help) {
  console.log(`Usage:
  bun run guardian-cards:materialize-choice
  bun run scripts/materialize-guardian-choice-editions.ts --check`)
  process.exit(0)
}
...
const blueprintPath =
  values.blueprint ??
  fileURLToPath(new URL('../content/guardian-cards/guardian-choice-edition-blueprints-ko.json', import.meta.url))
const outputPath =
  values.output ?? fileURLToPath(new URL('../content/guardian-cards/guardian-choice-editions-ko.json', import.meta.url))
...
try {
  ...
  if (values.write) {
    ...
    if (existing !== output) {
      throw new Error(
        `Choice edition materialization is stale: run bun run guardian-cards:materialize-choice and commit ${outputPath}`,
      )
    }
    console.log('checked: guardian-choice-editions-ko (192 editorial drafts, materialization current)')
  }
} catch (error) {
  console.error(error instanceof Error ? error.message : 'Choice edition materialization failed')
  process.exitCode = 1
}
...
function parseBlueprint(raw: string): Blueprint {
  ...
  try {
    ...
    throw new Error(`Invalid JSON in choice edition blueprint: ${blueprintPath}`)
  }
  ...
  throw new Error(`Invalid choice edition blueprint:\n- ${issues.join('\n- ')}`)
}
...
function validateBlueprint(blueprint: Blueprint): void {
  ...
  for (const family of blueprint.families) {
    if (family.familyId !== `${family.sign}.choice`) {
      errors.push(`${family.familyId}: family id must match ${family.sign}.choice`)
    }
    checkExactIds(
    ...
    for (const context of family.contexts) {
      ...
      for (const signal of context.selectionSignals) {
        if (!signal.startsWith('choice.')) {
          errors.push(`${family.familyId}.${context.id}: ${signal} is not a choice signal`)
        }
      }
    }
  }
  if (errors.length > 0) {
    throw new Error(`Choice edition blueprint is invalid:\n- ${errors.join('\n- ')}`)
  }
}
    ...
    family.contexts.flatMap((context) =>
      blueprint.treatments.map((treatment, treatmentIndex) => {
        ...
        return {
          ...
          guardians: treatment.id === 'shared-world' ? family.sharedGuardians : family.guardians,
