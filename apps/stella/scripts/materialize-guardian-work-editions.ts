...
const CONTEXTS = ['motivation', 'strength', 'pressure', 'next-move'] as const
...
if (values.help) {
  console.log(`Usage:
  bun run guardian-cards:materialize-work
  bun run scripts/materialize-guardian-work-editions.ts --check`)
  process.exit(0)
}
...
const blueprintPath =
  values.blueprint ??
  fileURLToPath(new URL('../content/guardian-cards/guardian-work-edition-blueprints-ko.json', import.meta.url))
const outputPath =
  values.output ?? fileURLToPath(new URL('../content/guardian-cards/guardian-work-editions-ko.json', import.meta.url))
...
try {
  ...
  if (values.write) {
    ...
    if (existing !== output) {
      throw new Error(
        `Work edition materialization is stale: run bun run guardian-cards:materialize-work and commit ${outputPath}`,
      )
    }
    console.log('checked: guardian-work-editions-ko (192 editorial drafts, materialization current)')
  }
} catch (error) {
  console.error(error instanceof Error ? error.message : 'Work edition materialization failed')
  process.exitCode = 1
}
...
function parseBlueprint(raw: string): Blueprint {
  ...
  try {
    ...
    throw new Error(`Invalid JSON in work edition blueprint: ${blueprintPath}`)
  }
  ...
  throw new Error(`Invalid work edition blueprint:\n- ${issues.join('\n- ')}`)
}
...
function validateBlueprint(blueprint: Blueprint): void {
  ...
  for (const family of blueprint.families) {
    if (family.familyId !== `${family.sign}.work`) {
      errors.push(`${family.familyId}: family id must match ${family.sign}.work`)
    }
    checkExactIds(
    ...
    for (const context of family.contexts) {
      ...
      for (const signal of context.selectionSignals) {
        if (!signal.startsWith('work.')) {
          errors.push(`${family.familyId}.${context.id}: ${signal} is not a work signal`)
        }
      }
    }
  }
  if (errors.length > 0) {
    throw new Error(`Work edition blueprint is invalid:\n- ${errors.join('\n- ')}`)
  }
}
