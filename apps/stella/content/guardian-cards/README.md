...
| `production-art-batch-013-review-ko.json`    | `visual_review_ready`    | 열세 번째 production 배치 신규 PNG 12개 보조 시각 QA·최종 시각 승인 대기    |
| `guardian-card-asset-contract.json`          | `delivery_contract`      | R2 버킷·객체 키·WebP 최적화·캐시 불변 계약                                  |
| `guardian-card-assets-ko.json`               | `release_candidate`      | 승인된 154개 WebP 배포 후보의 객체 키·원본/배포 SHA-256                     |
...
현재 파일럿 12개와 production 배치 12개의 신규 142개, 총 154개 승인 원본을 WebP quality 82·effort 6으로
최적화했다. 누적 배포 후보 합계는 25,943,006 bytes다.
...
1,044장이며 열두 production 배치의 신규 142개까지 승인·WebP 준비한 현재 잔여량은 902장이다. 이 목록은
...
`apps/stella/private/guardian-art-production/self-next-self-close-emotion`에 보관한다. 현재 신규 12개는
사람의 최종 시각 승인 대기 상태이며 승인 전에는 WebP 변환·R2 업로드를 진행하지 않는다.
...
- 1,056개를 정확히 한 번씩 포함하는 88개 production 배치와 누적 승인 WebP 154개를 제외한 현재 잔여
  902개 산식
...
bun --filter=@sobok/stella guardian-cards:prepare-art \
  ...
  --output private/guardian-art-release-154

bun --filter=@sobok/stella guardian-cards:validate-art-release --manifest \
  private/guardian-art-release-154/manifest.json

bun --filter=@sobok/stella guardian-cards:package-art-release --manifest \
  private/guardian-art-release-154/manifest.json --output \
  private/releases/guardian-art-release-154/guardian-card-art-release.tar.gz
```
