...
| `production-art-batch-036-review-ko.json`    | `editorial_review_complete` | 서른여섯 번째 production 배치 신규 12개 편집 승인·승인 해시 고정, PNG 병렬 생성 중 |
...
서른여섯 번째 배치는 `love.shared-play` 원고를 `stella` 희귀도로 제작할 신규 12개 편집 검토안이다.
...
canonical hash를 고정한 뒤 built-in imagegen으로 PNG 후보를 병렬 생성한다. WebP 최적화와
