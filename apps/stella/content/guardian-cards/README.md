...
| `production-art-batch-024-review-ko.json`    | `visual_review_ready`    | 스물네 번째 production 배치 신규 PNG 12개 사람 시각 검토 대기                 |
...
`apps/stella/private/guardian-art-production/love-careful-approach-stella`에 보관한다. 현재 신규 12개는
사람의 최종 시각 검토 대기 상태이며, 승인 전까지 누적 승인 WebP 285개와 잔여 771개 산식은 바뀌지 않는다.
...
- 스물네 번째 production 배치 신규 12개 원고 해시와 두 수호령이 독립 조절점을 유지한 채 다시 열 수
  ...
  후보·마스터/80px·이전 스물세 배치 비교 보조 시각 QA 및 사람의 최종 시각 승인 전 배포 후보 확정 차단
