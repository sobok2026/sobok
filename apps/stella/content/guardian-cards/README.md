...
| `production-art-batch-012-review-ko.json`    | `editorial_review_complete` | 열두 번째 production 배치 신규 12개 원고·비개인화 여운 구도 최종 편집 승인 완료 |
| `guardian-card-asset-contract.json`          | `delivery_contract`         | R2 버킷·객체 키·WebP 최적화·캐시 불변 계약                                      |
| `guardian-card-assets-ko.json`               | `release_candidate`         | 승인된 142개 WebP 배포 후보의 객체 키·원본/배포 SHA-256                         |
...
현재 파일럿 12개와 production 배치 11개의 신규 130개, 총 142개 승인 원본을 WebP quality 82·effort 6으로
최적화했다. 누적 배포 후보 합계는 24,113,804 bytes다.
...
1,044장이며 열한 production 배치의 신규 130개까지 승인·WebP 준비한 현재 잔여량은 914장이다. 이 목록은
...
깊이와 열린 여운 방향을 사용한다. 현재 12개 원고 해시와 구도는 2026-08-16 사람의 최종 편집 승인을
마쳤으며 imagegen은 이 승인 해시만 입력으로 사용한다.
...
- 1,056개를 정확히 한 번씩 포함하는 88개 production 배치와 누적 승인 WebP 142개를 제외한 현재 잔여
  914개 산식
...
bun --filter=@sobok/stella guardian-cards:prepare-art \
  ...
  --output private/guardian-art-release-142

bun --filter=@sobok/stella guardian-cards:validate-art-release --manifest \
  private/guardian-art-release-142/manifest.json

bun --filter=@sobok/stella guardian-cards:package-art-release --manifest \
  private/guardian-art-release-142/manifest.json --output \
  private/releases/guardian-art-release-142/guardian-card-art-release.tar.gz
```
