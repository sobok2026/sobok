...
| `production-art-batch-009-review-ko.json`    | `editorial_review_ready` | 아홉 번째 production 배치 신규 12개 원고·구도 최종 편집 승인 대기           |
| `guardian-card-asset-contract.json`          | `delivery_contract`      | R2 버킷·객체 키·WebP 최적화·캐시 불변 계약                                  |
| `guardian-card-assets-ko.json`               | `release_candidate`      | 승인된 106개 WebP 배포 후보의 객체 키·원본/배포 SHA-256                     |
...
현재 파일럿 12개와 production 배치 8개의 신규 94개, 총 106개 승인 원본을 WebP quality 82·effort 6으로
최적화했다. 누적 배포 후보 합계는 17,493,462 bytes다.
...
1,044장이며 여덟 production 배치의 신규 94개까지 승인·WebP 준비한 현재 잔여량은 950장이다. 이 목록은
...
지도, 별탑 모서리, 반투명 물결 시트, 얕은 바다 오버헤드처럼 서로 다른 근접 시점을 사용한다. 현재 원고
해시와 구도는 사람의 최종 편집 승인 대기 상태이며 승인 전에는 imagegen 원화 생성을 진행하지 않는다.
...
- 1,056개를 정확히 한 번씩 포함하는 88개 production 배치와 누적 승인 WebP 106개를 제외한 현재 잔여
  950개 산식
...
bun --filter=@sobok/stella guardian-cards:prepare-art \
  ...
  --output private/guardian-art-release-106

bun --filter=@sobok/stella guardian-cards:validate-art-release --manifest \
  private/guardian-art-release-106/manifest.json

bun --filter=@sobok/stella guardian-cards:package-art-release --manifest \
  private/guardian-art-release-106/manifest.json --output \
  private/releases/guardian-art-release-106/guardian-card-art-release.tar.gz
```
