...
| `production-art-batch-010-review-ko.json`    | `editorial_review_complete` | 열 번째 production 배치 신규 12개 원고·행동 구도 최종 편집 승인 완료        |
| `guardian-card-asset-contract.json`          | `delivery_contract`         | R2 버킷·객체 키·WebP 최적화·캐시 불변 계약                                  |
| `guardian-card-assets-ko.json`               | `release_candidate`         | 승인된 118개 WebP 배포 후보의 객체 키·원본/배포 SHA-256                     |
...
현재 파일럿 12개와 production 배치 9개의 신규 106개, 총 118개 승인 원본을 WebP quality 82·effort 6으로
최적화했다. 누적 배포 후보 합계는 19,511,772 bytes다.
...
1,044장이며 아홉 production 배치의 신규 106개까지 승인·WebP 준비한 현재 잔여량은 938장이다. 이 목록은
...
사람의 최종 편집 승인을 마쳤다. 이제 승인 해시만으로 imagegen 원화 후보를 생성할 수 있으며, 생성 후보는
사람의 최종 시각 승인 전까지 WebP 최적화와 R2 배포를 진행하지 않는다.
...
- 1,056개를 정확히 한 번씩 포함하는 88개 production 배치와 누적 승인 WebP 118개를 제외한 현재 잔여
  938개 산식
...
bun --filter=@sobok/stella guardian-cards:prepare-art \
  ...
  --output private/guardian-art-release-118

bun --filter=@sobok/stella guardian-cards:validate-art-release --manifest \
  private/guardian-art-release-118/manifest.json

bun --filter=@sobok/stella guardian-cards:package-art-release --manifest \
  private/guardian-art-release-118/manifest.json --output \
  private/releases/guardian-art-release-118/guardian-card-art-release.tar.gz
```
