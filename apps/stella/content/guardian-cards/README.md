...
| `production-art-batch-027-review-ko.json`    | `editorial_review_complete` | 스물일곱 번째 production 배치 신규 12개 편집 승인·원고 해시 고정              |
...
승인을 마쳤으며 imagegen은 이 승인 해시만 입력으로 사용한다. 아직 신규 이미지를 만들거나 배포
자산으로 편입하지 않았으므로 누적 승인 WebP 321개와 잔여 735개 산식은 바뀌지 않는다.
