...
| `production-art-batch-039-review-ko.json`    | `editorial_review_complete` | 서른아홉 번째 production 배치 신규 PNG 12개 생성 계약·승인 해시 고정                  |
...
서른아홉 번째 배치는 `love.boundary-and-space` 원고를 `eclipse` 희귀도로 제작할 신규 12개 편집
검토안이다. `production-art-batch-039-review-ko.json`에 현재 원고의 canonical SHA-256과 원고가 지정한
