...
| `production-art-batch-011-review-ko.json`    | `editorial_review_complete` | 열한 번째 production 배치 신규 12개 원고·관계 구도 최종 편집 승인 완료      |
...
현재 파일럿 12개와 production 배치 10개의 신규 118개, 총 130개 승인 원본을 WebP quality 82·effort 6으로
최적화했다. 누적 배포 후보 합계는 21,700,082 bytes다.
...
1,044장이며 열 production 배치의 신규 118개까지 승인·WebP 준비한 현재 잔여량은 926장이다. 이 목록은
...
초승달 바위턱처럼 서로 다른 관계축과 여백을 사용한다. 현재 원고 해시와 구도는 2026-08-16 사람의 최종
편집 승인을 마쳤으며 imagegen은 이 승인 해시만 입력으로 사용한다.
...
- 1,056개를 정확히 한 번씩 포함하는 88개 production 배치와 누적 승인 WebP 130개를 제외한 현재 잔여
  926개 산식
...
bun --filter=@sobok/stella guardian-cards:prepare-art \
  ...
  --output private/guardian-art-release-130

bun --filter=@sobok/stella guardian-cards:validate-art-release --manifest \
  private/guardian-art-release-130/manifest.json

bun --filter=@sobok/stella guardian-cards:package-art-release --manifest \
  private/guardian-art-release-130/manifest.json --output \
  private/releases/guardian-art-release-130/guardian-card-art-release.tar.gz
```
