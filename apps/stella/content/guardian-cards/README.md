...
| `production-art-batch-023-review-ko.json`    | `visual_review_ready`    | 스물세 번째 production 배치 신규 12개 Eclipse 원화 최종 시각 승인 대기        |
...
현재 파일럿 12개와 production 배치 22개의 신규 261개, 총 273개 승인 원본을 WebP quality 82·effort 6으로
최적화했다. 누적 배포 후보 합계는 49,270,200 bytes다.
...
1,044장이며 스물두 production 배치의 신규 261개까지 승인·WebP 준비한 현재 잔여량은 783장이다. 이 목록은
...
`apps/stella/private/guardian-art-production/love-careful-approach-eclipse`에 보관한다. 원화의 사람 시각
승인 전까지 누적 승인 WebP 273개와 잔여 783개 산식은 바뀌지 않는다.
...
- 스물세 번째 production 배치 신규 12개 원고 해시와 두 수호령이 각자의 선택권을 유지하며 속도를
  ...
  마스터/80px·이전 스물두 배치 비교 보조 시각 QA 완료 및 사람의 최종 시각 승인 대기
...
bun --filter=@sobok/stella guardian-cards:prepare-art \
  ...
  --output private/guardian-art-release-273

bun --filter=@sobok/stella guardian-cards:validate-art-release --manifest \
  private/guardian-art-release-273/manifest.json

bun --filter=@sobok/stella guardian-cards:package-art-release --manifest \
  private/guardian-art-release-273/manifest.json --output \
  private/releases/guardian-art-release-273/guardian-card-art-release.tar.gz
```
