...
| `production-art-batch-014-review-ko.json`    | `visual_review_ready`    | 열네 번째 production 배치 신규 PNG 12개 보조 시각 QA·최종 시각 승인 대기    |
...
현재 파일럿 12개와 production 배치 13개의 신규 154개, 총 166개 승인 원본을 WebP quality 82·effort 6으로
최적화했다. 누적 배포 후보 합계는 28,509,090 bytes다.
...
1,044장이며 열세 production 배치의 신규 154개까지 승인·WebP 준비한 현재 잔여량은 890장이다. 이 목록은
...
`apps/stella/private/guardian-art-production/self-next-self-action-beat`에 보관한다. 현재 신규 12개는
...
- 1,056개를 정확히 한 번씩 포함하는 88개 production 배치와 누적 승인 WebP 166개를 제외한 현재 잔여
  890개 산식
...
bun --filter=@sobok/stella guardian-cards:prepare-art \
  ...
  --output private/guardian-art-release-166

bun --filter=@sobok/stella guardian-cards:validate-art-release --manifest \
  private/guardian-art-release-166/manifest.json

bun --filter=@sobok/stella guardian-cards:package-art-release --manifest \
  private/guardian-art-release-166/manifest.json --output \
  private/releases/guardian-art-release-166/guardian-card-art-release.tar.gz
```
