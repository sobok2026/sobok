...
| `production-art-batch-032-review-ko.json`    | `editorial_review_complete` | 서른두 번째 production 배치 편집 승인·원고 해시 고정, 이미지 생성 대기        |
...
사람의 최종 편집 승인을 마쳤으며 현재 12개 모두 `approved`이고 canonical hash를 고정했다. 이 승인
해시를 imagegen 입력 경계로 사용해 PNG 후보를 만들며, 사람의 최종 시각 승인 전에는 WebP/R2 반영을
허용하지 않는다.
...
- 서른두 번째 production 배치 신규 12개 원고 해시와 솔직히 드러낸 내용·확인한 응답을 다시 열 수 있는
  ...
  비반복, 사람의 편집 승인·canonical hash 고정, 승인 해시 기반 imagegen 이전 단계·최종 시각 승인 전
  WebP/R2 반영 차단
