...
| `production-art-batch-028-review-ko.json`    | `visual_review_ready`    | 스물여덟 번째 production 배치 신규 PNG 12개 사람의 최종 시각 검토 대기        |
...
시트는 공개 Git에 포함하지 않는
`apps/stella/private/guardian-art-production/love-everyday-care-stella`에 보관한다. 현재 신규 12개는
사람의 최종 시각 검토 대기 상태이며 승인 전에는 WebP 배포 후보나 R2 객체로 편입하지 않는다.
...
- 스물여덟 번째 production 배치 신규 12개 원고 해시와 지정된 두 수호령이 각자의 조절점·선택권을
  ...
  스물일곱 배치 비교 보조 시각 QA 및 사람의 최종 시각 승인 전 배포 자산 편입 차단
