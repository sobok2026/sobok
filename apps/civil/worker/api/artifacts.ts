import { CIVIL_ARTIFACT_MAX_BYTES, CIVIL_ARTIFACT_PART_SIZE_BYTES } from '@sobok/civil/artifact'
...
import {
  ...
  listArtifacts,
  markArtifactInspectionDispatched,
  markArtifactInspectionDispatchFailed,
...
const CreateArtifactUploadBody = z
  .object({
    fileName: z.string().min(1).max(255).refine(isSafeFileName),
    mediaType: z
      .string()
      .trim()
      .min(1)
    ...
    return c.json(
      {
        ...
        upload: { ...result.upload, expiresAt: result.upload.expiresAt.toISOString() },
      },
      201,
      NO_STORE_HEADERS,
    )
...
artifacts.post('/:organizationId/projects/:projectId/artifacts/:artifactId/complete', async (c) => {
  ...
  let marked: Awaited<ReturnType<typeof markArtifactUploadCompleted>>
  try {
    marked = await withCivilSession(c, (db, session) => {
      if (!session) return Promise.resolve(null)
  ...
  try {
    ...
    await withCivilSession(c, (db, session) => {
      ...
      return markArtifactInspectionDispatched(db, {
        userId: session.user.id,
        organizationId: organizationId.data,
        projectId: projectId.data,
        artifactId: artifactId.data,
    ...
    await withCivilSession(c, (db, session) => {
      if (!session) return Promise.resolve()
      return markArtifactInspectionDispatchFailed(db, {
        userId: session.user.id,
        organizationId: organizationId.data,
        projectId: projectId.data,
        artifactId: artifactId.data,
  ...
  return c.json({ id: artifactId.data, status: 'quarantined' }, 202, NO_STORE_HEADERS)
})
...
artifacts.on(['GET', 'HEAD'], '/:organizationId/projects/:projectId/artifacts/:artifactId/download', async (c) => {
  ...
  return new Response(c.req.method === 'HEAD' || !hasR2Body(object) ? null : object.body, { status, headers })
})
