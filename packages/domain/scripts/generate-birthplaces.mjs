...
const OUTPUT_PATHS = Object.fromEntries(
  LOCALES.map((locale) => [locale, join(PACKAGE_DIR, `src/birthplace/${locale}.generated.ts`)]),
)
...
if (checkOnly) {
  ...
  if (staleLocales.length > 0) {
    throw new Error(
      `Generated birthplace catalogs are stale for ${staleLocales.join(', ')}; run \`bun run generate:birthplaces\``,
    )
  }
...
function serializeCatalog(locale, groups, catalogPlaces, sourceHashes) {
  ...
  return `// AUTO-GENERATED by packages/domain/scripts/generate-birthplaces.mjs — do not edit.
// Source and license details: packages/domain/LICENSES/AdministrativeBirthplaces.txt and GeoNames.txt.
${sourceLines}
// ${catalogPlaces.length} selectable places. Regenerate with \`bun run generate:birthplaces\` from packages/domain.
...
export const GENERATED_BIRTHPLACE_CATALOG = createBirthplaceCatalog('${locale}', GROUPS, PLACES)
`
}
